...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Obtener un campeonato específico, o el primer campeonato disponible
            campeonato = Campeonato.objects.first()  # O alguna lógica para seleccionar un campeonato

            # Redirigir según el tipo de usuario
            if user.is_superuser or user.groups.filter(name='Administrador').exists():
                return redirect('gestionar_registros')
            elif user.groups.filter(name='Planillero').exists():
                # Redirigir al planillero a la lista de partidos del primer campeonato
